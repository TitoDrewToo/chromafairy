  ...
  return (
    <div className="admin-dashboard admin-operations-page admin-insights-page">
      ...
      <section className="admin-insight-block" aria-labelledby="attention-heading">
        ...
        {trendViews.length >= 6 || trendInquiries.length >= 6 ? <div className="admin-insight-trends">{trendViews.length >= 6 ? <BarTrend points={points} valueKey="views" title="Views over time" valueLabel="views" /> : null}{trendInquiries.length >= 6 ? <BarTrend points={points} valueKey="inquiries" title="Inquiries over time" valueLabel="inquiries" /> : null}</div> : null}
      </section>
...
function Sparkline({ points, label }: { points: number[]; label: string }) {
  const populated = points.filter((value) => value > 0);
  if (populated.length < 2) return <svg className="admin-sparkline" viewBox="0 0 120 34" role="img" aria-label={label} />;
  const max = Math.max(1, ...points);
  const width = 120;
  const height = 34;
  const step = width / (points.length - 1);
  const path = points.map((value, index) => `${index ? "L" : "M"}${(index * step).toFixed(2)},${(height - (value / max) * (height - 3)).toFixed(2)}`).join(" ");
  const fill = `${path} L ${width},${height} L 0,${height} Z`;
  const endIndex = points.length - 1;
  const endValue = points[endIndex] ?? 0;
  const endX = endIndex * step;
  const endY = height - (endValue / max) * (height - 3);
  const gradientId = `insights-spark-${label.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
  return <svg className="admin-sparkline" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={label}><defs><linearGradient id={gradientId} x1="0%" x2="100%"><stop offset="0%" stopColor="#e4b84f" /><stop offset="25%" stopColor="#ff66c4" /><stop offset="50%" stopColor="#57e8ff" /><stop offset="75%" stopColor="#9cff57" /><stop offset="100%" stopColor="#ff66c4"><animate attributeName="offset" values="90%;100%;90%" dur="4.8s" repeatCount="indefinite" /></stop></linearGradient></defs><path className="admin-spark-fill" d={fill} style={{ fill: `url(#${gradientId})` }} /><path d={path} style={{ stroke: `url(#${gradientId})` }} /><circle className="admin-spark-end" cx={endX} cy={endY} r="2.5" style={{ fill: `url(#${gradientId})` }}><title>{`${label}, ${formatNumber(endValue)}`}</title></circle></svg>;
}
