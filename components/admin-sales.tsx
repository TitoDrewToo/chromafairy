...
export default function SalesAdmin({ works, customers: initialCustomers, inquiries, initialOrders }: { works: SaleWork[]; customers: SaleCustomer[]; inquiries: SaleInquiry[]; initialOrders: AdminOrder[] }) {
  ...
  return <section className="admin-sales-layout">
    <form className="admin-operation-form" onSubmit={submit}><Hint id="recordSale"><h2>Record a sale</h2></Hint>
      <Hint id="salesCustomer"><label>Existing customer<select value={customerId} onChange={(event) => chooseCustomer(event.target.value)}><option value="">New or match by email</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name || customer.email}</option>)}</select></label></Hint>
      <div className="admin-two-col"><Hint id="customerName"><label>Name<input required value={customerName} onChange={(event) => setCustomerName(event.target.value)} /></label></Hint><Hint id="customerEmail"><label>Email<input required type="email" value={customerEmail} onChange={(event) => setCustomerEmail(event.target.value)} /></label></Hint></div>
      <Hint id="customerPhone"><label>Phone<input value={customerPhone} onChange={(event) => setCustomerPhone(event.target.value)} /></label></Hint>
      ...
      <div className="admin-two-col"><Hint id="finalAmount"><label>Final amount<input required min="0" step="0.01" type="number" value={amount} onChange={(event) => setAmount(event.target.value)} /></label></Hint><Hint id="currency"><label>Currency<select value={currency} onChange={(event) => setCurrency(event.target.value)}><option>PHP</option><option>USD</option><option>EUR</option></select></label></Hint></div>
      <div className="admin-two-col"><Hint id="saleDate"><label>Sale date<input required type="date" value={saleDate} onChange={(event) => setSaleDate(event.target.value)} /></label></Hint><Hint id="channel"><label>Channel<input value={channel} onChange={(event) => setChannel(event.target.value)} /></label></Hint></div>
      <Hint id="originatingInquiry"><label>Originating inquiry<select value={inquiryId} onChange={(event) => setInquiryId(event.target.value)}><option value="">None</option>{inquiries.map((inquiry) => <option key={inquiry.id} value={inquiry.id}>{inquiry.name} · {inquiry.work_title_snapshot ?? "Commission"}</option>)}</select></label></Hint>
      <Hint id="notes"><label>Notes<textarea rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} /></label></Hint>
      <Hint id="shipment"><label className="admin-check"><input checked={showShipment} onChange={(event) => setShowShipment(event.target.checked)} type="checkbox" /> Add manual shipment</label></Hint>
      {showShipment && <div className="admin-two-col"><Hint id="carrier"><label>Carrier<input value={carrier} onChange={(event) => setCarrier(event.target.value)} /></label></Hint><Hint id="tracking"><label>Tracking<input value={trackingNumber} onChange={(event) => setTrackingNumber(event.target.value)} /></label></Hint><Hint id="package"><label>Package<select value={packageType} onChange={(event) => setPackageType(event.target.value as PackageType)}><option value="flat">Flat</option><option value="rolled_tube">Rolled tube</option><option value="crate">Crate</option></select></label></Hint></div>}
    ...
    <div className="admin-order-list"><Hint id="recordSale"><h2>Orders</h2></Hint>{orders.length ? orders.map((order) => <article className="admin-order-card" key={order.id}><div><strong>{order.work?.title ?? "Work removed"}</strong><span>{order.customer?.name ?? order.buyer_name ?? order.buyer_email} · {order.sale_date}</span></div><b>{formatAmount(order.amount, order.currency)}</b><Hint id="orderStatus"><select aria-label={`Order status for ${order.id}`} value={order.order_status ?? "paid"} onChange={(event) => void updateOrder(order.id, event.target.value as OrderStatus)}>{orderStatuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></Hint>{order.work && <Hint id="viewWork"><Link href={`/shop/${order.work.slug}`}>View work</Link></Hint>}</article>) : <p className="admin-empty-state">No sales recorded yet.</p>}</div>
  </section>;
}
