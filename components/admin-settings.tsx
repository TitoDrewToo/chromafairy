...
export default function SettingsAdmin({ initialFlags }: { initialFlags: FeatureFlag[] }) {
  ...
  return <section className="admin-flag-list">{error && <p className="admin-error" role="alert">{error}</p>}{FLAG_GROUPS.map((group) => <section className="admin-flag-group" key={group.label}><h2 className="admin-flag-group-label">{group.label}</h2>{group.flags.map(([key, label]) => { const flag = flags.find((item) => item.key === key); if (!flag) return null; const hintId = flag.key === "payments" ? "payments" : flag.key === "shipping_automation" ? "shippingAutomation" : flag.key === "self_booking" ? "selfBooking" : "calendarSync"; return <article className="admin-flag-card" key={flag.key}><div><strong>{label}</strong><p>{flag.notes}</p></div><Hint id={hintId}><button aria-pressed={flag.enabled} className={`admin-flag-toggle ${flag.enabled ? "is-on" : ""}`} onClick={() => void toggle(flag)} type="button">{flag.enabled ? "On" : "Off"}</button></Hint></article>; })}</section>)}</section>;
}
