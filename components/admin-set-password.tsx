...
export default function SetPasswordForm({ mode }: { mode: "forgot" | "invite" | "reset" }) {
  ...
  return mode === "forgot" ? <form className="admin-login-form" onSubmit={requestReset}><Hint id="resetEmail"><label>Email<input autoComplete="email" required type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label></Hint>{error && <p className="admin-error" role="alert">{error}</p>}{message && <p className="admin-inline-success" role="status">{message}</p>}<Hint id="sendReset"><button className="admin-action-button admin-primary-button" disabled={busy} type="submit"><span className="admin-action-label">{busy ? "Sending…" : "Send reset link"}</span></button></Hint><Hint id="backToLogin"><Link className="admin-back-link" href="/studio/login">Back to sign in</Link></Hint></form> : <form className="admin-login-form" onSubmit={setNewPassword}><Hint id="password"><label>New password<input autoComplete="new-password" minLength={8} required type="password" value={password} onChange={(event) => setPassword(event.target.value)} /></label></Hint><Hint id="password"><label>Confirm password<input autoComplete="new-password" minLength={8} required type="password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></label></Hint>{error && <p className="admin-error" role="alert">{error}</p>}<Hint id="password"><button className="admin-action-button admin-primary-button" disabled={busy} type="submit"><span className="admin-action-label">{busy ? "Saving…" : "Set password"}</span></button></Hint><Hint id="backToLogin"><Link className="admin-back-link" href="/studio/login">Back to sign in</Link></Hint></form>;
}
