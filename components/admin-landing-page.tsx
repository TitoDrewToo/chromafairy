...
export default function AdminLandingPage({ initialContent }: { initialContent: LandingPageContent }) {
  ...
  return <section className="admin-landing-editor">
    ...
    {sections.map((section) => <section className="admin-landing-section" key={section.id}>
      <div className="admin-landing-section-heading"><div><p className="admin-eyebrow">{sectionLabels[section.section_key]}</p><h2>{section.title || sectionLabels[section.section_key]}</h2><p className="admin-muted">{section.items.length} / {limits[section.section_key]} slots used</p></div><Hint id="landingPublish"><label className="admin-landing-publish"><input checked={section.is_published} onChange={(event) => updateSection(section.id, { is_published: event.target.checked })} type="checkbox" /> Published</label></Hint></div>
      <div className="admin-landing-copy"><Hint id="landingEyebrow"><label>Eyebrow<input value={section.eyebrow} onChange={(event) => updateSection(section.id, { eyebrow: event.target.value })} /></label></Hint><Hint id="landingTitle"><label>Title<input value={section.title} onChange={(event) => updateSection(section.id, { title: event.target.value })} /></label></Hint><Hint id="landingDescription"><label className="field-wide">Description<textarea rows={3} value={section.body} onChange={(event) => updateSection(section.id, { body: event.target.value })} /></label></Hint></div>
      <Hint id="landingSaveSection"><button className="admin-action-button" disabled={busy === `section-${section.id}`} onClick={() => void saveSection(section)} type="button">{busy === `section-${section.id}` ? "Saving…" : `Save ${sectionLabels[section.section_key]} copy`}</button></Hint>
      ...
      {section.section_key !== "gallery" && <Hint id="landingAddItem"><button className="admin-secondary-button admin-landing-add" disabled={section.items.length >= limits[section.section_key] || Boolean(busy)} onClick={() => addItem(section)} type="button">+ Add {section.section_key === "collections" ? "series" : section.section_key === "press" ? "press entry" : "exhibition"}</button></Hint>}
    </section>)}
  </section>;
}
...
function LandingItemEditor({ section, entry, index, busy, onChange, onImage, onMove, onRemove, onSave }: { section: LandingPageContent["sections"][number]; entry: LandingItem; index: number; busy: string; onChange: (patch: Partial<LandingItem>) => void; onImage: (index: number, file: File) => void; onMove: (direction: -1 | 1) => void; onRemove: () => void; onSave: () => void }) {
  ...
  return <article className="admin-landing-item">
    <div className="admin-landing-item-heading"><strong>{index + 1}. {entry.title || "Untitled entry"}</strong><div className="admin-landing-item-actions"><Hint id="landingReorder"><button className="admin-small-button" disabled={index === 0 || Boolean(busy) || !entry.created_at || section.items.some((item) => !item.created_at)} onClick={() => onMove(-1)} title={!entry.created_at ? "Save this entry before reordering" : undefined} type="button">↑</button></Hint><Hint id="landingReorder"><button className="admin-small-button" disabled={index === section.items.length - 1 || Boolean(busy) || !entry.created_at || section.items.some((item) => !item.created_at)} onClick={() => onMove(1)} title={!entry.created_at ? "Save this entry before reordering" : undefined} type="button">↓</button></Hint>{section.section_key !== "gallery" && <Hint id="landingRemove"><button className="admin-small-button admin-danger-button" disabled={Boolean(busy)} onClick={onRemove} type="button">Remove</button></Hint>}</div></div>
    {section.section_key === "press" && <Hint id="landingFormat"><label>Entry format<select value={entry.item_type} onChange={(event) => onChange({ item_type: event.target.value as LandingItemType, media: event.target.value === "press_text" ? [] : entry.media })}><option value="press_image">Image feature</option><option value="press_text">Text / link</option></select></label></Hint>}
    <div className="admin-landing-item-fields"><Hint id="landingItemEyebrow"><label>Eyebrow<input value={entry.eyebrow} onChange={(event) => onChange({ eyebrow: event.target.value })} /></label></Hint><Hint id="landingItemTitle"><label>Title<input value={entry.title} onChange={(event) => onChange({ title: event.target.value })} /></label></Hint><Hint id="landingItemSubtitle"><label>Subtitle / metadata<textarea rows={2} value={entry.subtitle} onChange={(event) => onChange({ subtitle: event.target.value })} /></label></Hint><Hint id="landingItemCopy"><label>Copy<textarea rows={3} value={entry.body} onChange={(event) => onChange({ body: event.target.value })} /></label></Hint><Hint id="landingSource"><label>Source<input value={entry.source} onChange={(event) => onChange({ source: event.target.value })} /></label></Hint><Hint id="landingLinkUrl"><label>Link URL or blog path<input placeholder="https://… or /blog/your-slug" value={entry.link_url} onChange={(event) => onChange({ link_url: event.target.value })} /></label></Hint><Hint id="landingLinkLabel"><label>Link label<input value={entry.link_label} onChange={(event) => onChange({ link_label: event.target.value })} /></label></Hint><Hint id="landingItemPublish"><label className="admin-landing-publish"><input checked={entry.is_published} onChange={(event) => onChange({ is_published: event.target.checked })} type="checkbox" /> Published</label></Hint></div>
    ...
    <Hint id="landingSaveItem"><button className="admin-action-button" disabled={Boolean(busy)} onClick={onSave} type="button">{busy === `item-${entry.id}` ? "Saving…" : "Save entry"}</button></Hint>
  </article>;
}
...
function LandingMediaEditor({ entry, index, busy, onChange, onImage }: { entry: LandingItem; index: number; busy: string; onChange: (patch: Partial<LandingItem>) => void; onImage: (index: number, file: File) => void }) {
  ...
  return <div className="admin-landing-media"><div className="admin-landing-media-preview">{media.path ? <img alt={media.alt || "Landing image"} src={media.path.startsWith("/") || /^https?:\/\//i.test(media.path) ? media.path : client ? getArtworkUrl(client, media.path) : media.path} /> : <span>No image</span>}</div><Hint id="landingImage"><label>Image<input accept=".jpg,.jpeg,.png,.webp,.gif,.heic,.heif,image/jpeg,image/png,image/webp,image/gif,image/heic,image/heif" disabled={Boolean(busy)} onChange={(event) => { const file = event.target.files?.[0]; if (file) onImage(index, file); event.currentTarget.value = ""; }} type="file" /></label></Hint><Hint id="landingImageLabel"><label>Image label<input value={media.label} onChange={(event) => { const next = [...entry.media]; next[index] = { ...media, label: event.target.value }; onChange({ media: next }); }} /></label></Hint><Hint id="landingAltText"><label>Alt text<input value={media.alt} onChange={(event) => { const next = [...entry.media]; next[index] = { ...media, alt: event.target.value }; onChange({ media: next }); }} /></label></Hint></div>;
}
