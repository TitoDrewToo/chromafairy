...
export default function InquiryAdmin({ initialInquiries, archived = false }: { initialInquiries: AdminInquiry[]; archived?: boolean }) {
  ...
  const [statusFilter, setStatusFilter] = useState<InquiryStatus | "all">("new");
  ...
  return <section className="admin-inquiry-workspace">
    ...
    <div className="admin-inquiry-toolbar">
      <Hint id="inquirySearch"><label className="admin-inquiry-search">Search inquiries<input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name, email, message or work" /></label></Hint>
      <div className="admin-inquiry-filters" aria-label="Inquiry filters">
        <Hint id="inquiryView"><label>View<select value={archiveFilter} onChange={(event) => setArchiveFilter(event.target.value as ArchiveFilter)}><option value="active">Active</option><option value="archived">Archived</option><option value="all">All</option></select></label></Hint>
        <Hint id="inquiryType"><label>Type<select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as TypeFilter)}><option value="all">All types</option><option value="piece">Piece</option><option value="commission">Commission</option></select></label></Hint>
        <Hint id="inquiryStatus"><label>Status<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as InquiryStatus | "all")}><option value="all">All statuses</option>{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label></Hint>
        <Hint id="inquirySort"><button className="admin-action-button admin-inquiry-sort" type="button" onClick={() => setSortNewest((current) => !current)}>Date: {sortNewest ? "newest first" : "oldest first"}</button></Hint>
        {hasFilters && <Hint id="inquiryClear"><button className="admin-small-button" type="button" onClick={clearFilters}>Clear all</button></Hint>}
      </div>
    </div>
