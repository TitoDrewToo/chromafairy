...
export default function CustomersAdmin({ initialCustomers }: { initialCustomers: AdminCustomer[] }) {
  ...
  return <section className="admin-customer-section"><button className="admin-action-button" onClick={() => start()} type="button"><span className="admin-action-label">Add customer</span></button>{message && <p className="admin-inline-success" role="status">{message}</p>}{error && <p className="admin-error" role="alert">{error}</p>}{editing && <form className="admin-operation-form admin-customer-form" onSubmit={save}><h2>{editing === "new" ? "Add customer" : "Edit customer"}</h2><label>Name<input required value={name} onChange={(event) => setName(event.target.value)} /></label><label>Email<input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label><label>Phone<input value={phone} onChange={(event) => setPhone(event.target.value)} /></label><label>Notes<textarea rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} /></label><div className="admin-form-actions"><button className="admin-action-button" type="submit"><span className="admin-action-label">Save customer</span></button><button className="admin-secondary-button" onClick={() => setEditing(null)} type="button">Cancel</button></div></form>}
    <div className="admin-customer-list">{customers.map((customer) => { const total = customer.orders.reduce((sum, order) => sum + Number(order.amount ?? 0), 0); return <article className="admin-customer-card" key={customer.id}><div className="admin-customer-heading"><div><h2>{customer.name || "Unnamed customer"}</h2><p>{customer.email}{customer.phone ? ` · ${customer.phone}` : ""}</p></div><button className="admin-small-button" onClick={() => start(customer)} type="button">Edit</button></div><div className="admin-customer-total">{customer.orders.length} purchase{customer.orders.length === 1 ? "" : "s"} · PHP {total.toLocaleString("en-PH", { minimumFractionDigits: 2 })}</div>{customer.orders.length > 0 && <ul>{customer.orders.map((order) => <li key={order.id}>{order.work?.title ?? "Work removed"} · {order.sale_date} · {order.currency ?? "PHP"} {Number(order.amount ?? 0).toLocaleString("en-PH", { minimumFractionDigits: 2 })}</li>)}</ul>}</article>; })}</div>
  </section>;
}
