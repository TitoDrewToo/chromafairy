...
export default function CustomersAdmin({ initialCustomers }: { initialCustomers: AdminCustomer[] }) {
  ...
  return <section className="admin-customer-section"><Hint id="addCustomer"><button className="admin-action-button" onClick={() => start()} type="button"><span className="admin-action-label">Add customer</span></button></Hint>{message && <p className="admin-inline-success" role="status">{message}</p>}{error && <p className="admin-error" role="alert">{error}</p>}{editing && <form className="admin-operation-form admin-customer-form" onSubmit={save}><h2>{editing === "new" ? "Add customer" : "Edit customer"}</h2><Hint id="customerName"><label>Name<input required value={name} onChange={(event) => setName(event.target.value)} /></label></Hint><Hint id="customerEmail"><label>Email<input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label></Hint><Hint id="customerPhone"><label>Phone<input value={phone} onChange={(event) => setPhone(event.target.value)} /></label></Hint><Hint id="notes"><label>Notes<textarea rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} /></label></Hint><div className="admin-form-actions"><Hint id="saveCustomer"><button className="admin-action-button" type="submit"><span className="admin-action-label">Save customer</span></button></Hint><Hint id="cancel"><button className="admin-secondary-button" onClick={() => setEditing(null)} type="button">Cancel</button></Hint></div></form>}
