  ...
  return (
    <form className="admin-work-form" onSubmit={submit}>
      ...
      <section className="admin-form-section"><h2>Pricing & status</h2><div className="admin-form-grid">
        ...
        <Hint id="workFlags"><label className="admin-check"><input checked={priceOnRequest} onChange={(event) => setPriceOnRequest(event.target.checked)} type="checkbox" /> Price on request</label></Hint><Hint id="workFlags"><label className="admin-check"><input checked={isNew} onChange={(event) => setIsNew(event.target.checked)} type="checkbox" /> Mark as new</label></Hint><Hint id="workFlags"><label className="admin-check"><input checked={isFeatured} onChange={(event) => setIsFeatured(event.target.checked)} type="checkbox" /> Featured</label></Hint>
      </div></section>

      <section className="admin-form-section"><Hint id="shippingFields"><h2>Shipping (optional)</h2></Hint><div className="admin-form-grid"><Hint id="shipping"><label>Packed weight kg<input min="0" step="0.01" type="number" value={packedWeight} onChange={(event) => setPackedWeight(event.target.value)} /></label></Hint><Hint id="shipping"><label>Packed length<input min="0" step="0.01" type="number" value={packedL} onChange={(event) => setPackedL(event.target.value)} /></label></Hint><Hint id="shipping"><label>Packed width<input min="0" step="0.01" type="number" value={packedW} onChange={(event) => setPackedW(event.target.value)} /></label></Hint><Hint id="shipping"><label>Packed height<input min="0" step="0.01" type="number" value={packedH} onChange={(event) => setPackedH(event.target.value)} /></label></Hint><Hint id="shipping"><label className="admin-check"><input checked={shipRolled} onChange={(event) => setShipRolled(event.target.checked)} type="checkbox" /> Can ship rolled</label></Hint></div></section>

      <section className="admin-form-section"><Hint id="images"><h2>Images</h2></Hint><Hint id="imageUpload"><div className={`admin-dropzone ${dragging ? "is-dragging" : ""}`} onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDragOver={(event) => event.preventDefault()} onDrop={dropFiles}><strong>Drop artwork images here</strong><span>or choose files below</span><input accept=".jpg,.jpeg,.png,.webp,.gif,.heic,.heif,image/jpeg,image/png,image/webp,image/gif,image/heic,image/heif" multiple onChange={(event) => { void addFiles(event.target.files ?? []); event.currentTarget.value = ""; }} type="file" /></div></Hint>{imageDrafts.length > 0 && <div className="admin-image-list">{imageDrafts.map((image, index) => <div className="admin-image-item" key={image.id}><DraftImagePreview alt={image.alt ?? title} src={image.previewUrl ?? image.url} fileName={image.file?.name} /><div><strong>{index + 1}. {image.alt || "Artwork image"}</strong><div className="admin-image-actions"><Hint id="imagePrimary"><button className="admin-small-button" onClick={() => setPrimary(image.id)} type="button">{image.is_primary ? "Primary" : "Make primary"}</button></Hint><Hint id="imageMove"><button className="admin-small-button" disabled={index === 0} onClick={() => moveImage(index, -1)} type="button">↑</button></Hint><Hint id="imageMove"><button className="admin-small-button" disabled={index === imageDrafts.length - 1} onClick={() => moveImage(index, 1)} type="button">↓</button></Hint><Hint id="imageRemove"><button className="admin-small-button admin-danger-button" onClick={() => removeImage(image)} type="button">Remove</button></Hint></div></div></div>)}</div>}{!hasPrimary && imageDrafts.length > 0 && <p className="admin-form-note">The first image will be used as primary.</p>}</section>
